...
df_primap_emissions_expanded <- read_rds(paste0(path_results,"/df_primap_emissions_expanded.rds"))
...
# STARTING POINT : EMISSION in CRF FORMAT   ------------------------------------
#   with 3 variants : Edgar, Primap CR and Primap TP

df_ipcc_emissions <- bind_rows(df_edgar_emissions %>%
                                 mutate(scenario = "EDGAR"),
                               df_primap_emissions_expanded %>%
                                 rename(ghg_emission = primap_ghg_emission))


# SPLIT HOUSEHOLDS vs. INDUSTRIES ----------------------------------------------

# Split HH vs industries
df_ipcc_emissions_HH_vs_industries <- df_ipcc_emissions %>%
...
df_ipcc_emissions_HH_vs_industries_NA <- df_ipcc_emissions_HH_vs_industries %>%
  filter(is.na(industries_coeff))

if (nrow(df_ipcc_emissions_HH_vs_industries_NA) > 0) stop("Bridge table CRF -> NACE / HH incomplete")

# Total HH results
df_ipcc_emissions_HH <- df_ipcc_emissions_HH_vs_industries %>%
  filter(HH_coeff > 0) %>%
  mutate(ghg_emission = HH_coeff * ghg_emission) %>%
  select(scenario, emis_country, ipcc_code_2006, gas_type, time_period, ghg_emission)

df_estimated_aea_HH <- df_ipcc_emissions_HH %>%
  group_by(scenario, time_period, gas_type, emis_country) %>%
  summarise(ghg_emission = sum(ghg_emission)) %>%
  ungroup() %>%
  mutate(emis_industry = "HH")

# Total industries results
df_ipcc_emissions_industries <- df_ipcc_emissions_HH_vs_industries %>%
...
df_estimated_aea_industries <- df_ipcc_emissions_industries %>%
  ...
  group_by(scenario, emis_country, ipcc_code_2006, gas_type, time_period) %>%
...
df_estimated_aea_a64_before_air_sea_transp <- df_estimated_aea_industries %>%
  group_by(scenario, time_period, gas_type, emis_country, emis_industry) %>%
  summarise(ghg_emission = sum(ghg_emission)) %>%
  ungroup()
...
#   Please note that PRIMAP does not provide a "SEA" total - the allocation is based on EDGAR alone
df_H50_international <- df_ipcc_emissions %>%
  # World total "marine bunker" in EDGAR
  filter(scenario == "EDGAR",
         emis_country == "EDGAR_SEA", gas_type == "CO2",
         time_period %in% vc_estimation_years) %>%
...
#    we keep the same yoy % change as in the edgar or primap dataset

df_H51_from_1.A.3.a <- df_estimated_aea_a64_before_air_sea_transp %>%
  filter(emis_industry == "H51_from_1.A.3.a") %>%
  rename(from_ipcc_value = ghg_emission) %>%
  left_join(df_oecd_air_transport, by = c("time_period", "emis_country")) %>%
  group_by(scenario, emis_country) %>%
  mutate(ghg_emission = if_else(!is.na(oecd_value),
                                oecd_value,
                                from_ipcc_value * sum(oecd_value * (time_period == "2013"), na.rm = TRUE) / sum(from_ipcc_value * (time_period == "2013")))) %>%
  ungroup()
...
df_estimated_aea_a64 <- df_estimated_aea_a64_before_air_sea_transp %>%
  ...
  bind_rows(df_H50_international %>%
              ...
              expand_grid(scenario = c("EDGAR", "HISTCR", "HISTTP")) %>%
              select(scenario, time_period, gas_type, emis_country, emis_industry, ghg_emission)) %>%
...
write_rds(df_global_aea,
          paste0(path_results,"/global_aea_based_on_edgar_and_primap.rds"))
