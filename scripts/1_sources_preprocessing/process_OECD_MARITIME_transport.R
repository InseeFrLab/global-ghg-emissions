# SOURCE CONTENT : OECD Maritime transport CO2 emissions
...
df_oecd_maritime_transport_raw <- read_csv(paste0(local_path_sources, "/OECD_maritime_transport/OECD_maritime_transport_download_27_01_2025.csv"))

# ------------------------------ FORMATTING ------------------------------------

# Data formatting, countries at the 'alpha 3' level
df_oecd_maritime_transport_alpha3 <- df_oecd_maritime_transport_raw %>%
  mutate(time_period = as.character(TIME_PERIOD),
         value = OBS_VALUE / 1000) %>% # passage en kT
  select(REF_AREA, time_period, value) %>%
  left_join(df_oecd_figaro_codes_countries, by = c("REF_AREA" = "country_code_oecd")) %>%
  mutate(country_code_figaro = if_else(REF_AREA == "W", "WORLD", country_code_figaro))

# WORLD total excluding FIGARO 'Rest of the world'
df_oecd_maritime_transport_WORLD_excl_FIGW1 <- df_oecd_maritime_transport_alpha3 %>%
  filter(!is.na(country_code_figaro), country_code_figaro != "WORLD") %>%
  group_by(time_period) %>%
  summarise(value_excl_FIGW1 = sum(value)) %>%
  ungroup()

# Computing FIGARO 'rest of the world' as a residual
df_oecd_maritime_transport_FIGW1 <- df_oecd_maritime_transport_alpha3 %>%
  filter(country_code_figaro == "WORLD") %>%
  select(time_period, value_world = value) %>%
  left_join(df_oecd_maritime_transport_WORLD_excl_FIGW1, by = "time_period") %>%
  mutate(value_FIGW1 = value_world - value_excl_FIGW1)

# Row-binding : all FIGARO countries + 'Rest of the World'
df_oecd_maritime_transport <- df_oecd_maritime_transport_alpha3 %>%
  filter(!is.na(country_code_figaro), country_code_figaro != "WORLD") %>%
  bind_rows(df_oecd_maritime_transport_FIGW1 %>%
...
write_rds(df_oecd_maritime_transport, paste0(path_frmt_source, "/oecd_maritime_transport.rds"))
